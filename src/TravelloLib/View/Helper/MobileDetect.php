 ...
 * Mobile detect
 * 
 * Detects if user is mobile, tablet or desktop, uses class Mobile_Detect from google
 * 
 * @package    TravelloLib
 * @author     Ralf Eggert <r.eggert@travello.de>
 * @copyright  Copyright (c) 2012 Travello GmbH
...
class MobileDetect extends AbstractHelper
{
    ...
    {
        if (!isset(self::$_detect)) {
            self::$_detect = new Mobile_Detect();
        }
        
        switch ($mode) {
            case 'desktop':
                return !self::$_detect->isMobile() && !self::$_detect->isTablet();
                break;

            case 'tablet':
                return self::$_detect->isTablet();
                break;

            default:
            case 'mobile':
                return self::$_detect->isMobile();
        }
    }
}
